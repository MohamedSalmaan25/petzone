...
window.addEventListener("load", function () {
  // Run only if the window width is larger than 767px (for responsive behavior)
  if (window.innerWidth > 767) {
    const slides = document.querySelectorAll(".slider .slide-content");
    if (slides.length > 0) {
      theChecker(); // Initialize slider
      playSlider(); // Start auto-play if desired
    }
  }
  // Uncomment if you have another function like getTrendingProducts
  // getTrendingProducts();
});
...
function theChecker() {
  ...
  // Handle the Prev and Next button states
  if (currentSlide === 1) {
...
function playSlider() {
  ...
  // Ensure that only one interval is set at a time
